...
var getMostCommonRangeMap = weakMemoize(function(allPackages) {
  var dependencyRangesMapping = new Map(), _iteratorNormalCompletion2 = !0, _didIteratorError2 = !1, _iteratorError2 = void 0;
  try {
    ...
    _step2$value[1]), _i2 = 0, _NORMAL_DEPENDENCY_TY = NORMAL_DEPENDENCY_TYPES; _i2 < _NORMAL_DEPENDENCY_TY.length; _i2++) {
      ...
      if (deps) for (var depName in deps) {
        var depSpecifier = deps[depName];
        if (!allPackages.has(depName)) {
          if (!semver.validRange(deps[depName])) continue;
          var dependencyRanges = dependencyRangesMapping.get(depName) || {}, specifierCount = dependencyRanges[depSpecifier] || 0;
          dependencyRanges[depSpecifier] = specifierCount + 1, dependencyRangesMapping.set(depName, dependencyRanges);
        }
      }
    }
  } catch (err) {
  ...
  var mostCommonRangeMap = new Map(), _iteratorNormalCompletion3 = !0, _didIteratorError3 = !1, _iteratorError3 = void 0;
  try {
    for (var _step3, _iterator3 = dependencyRangesMapping[Symbol.iterator](); !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done); _iteratorNormalCompletion3 = !0) {
      var _step3$value = _slicedToArray(_step3.value, 2), _depName = _step3$value[0], specifierMap = _step3$value[1], specifierMapEntryArray = Object.entries(specifierMap), first = _slicedToArray(specifierMapEntryArray, 1)[0], maxValue = specifierMapEntryArray.reduce(function(acc, value) {
        return acc[1] === value[1] ? [ sembear.highest([ acc[0], value[0] ]), acc[1] ] : acc[1] > value[1] ? acc : value;
      }, first);
      mostCommonRangeMap.set(_depName, maxValue[0]);
    }
  } catch (err) {
...
var EXTERNAL_MISMATCH = makeCheck({
  validate: function(workspace, allWorkspace) {
    var errors = [], mostCommonRangeMap = getMostCommonRangeMap(allWorkspace), _iteratorNormalCompletion = !0, _didIteratorError = !1, _iteratorError = void 0;
    try {
      for (var _step, _iterator = NORMAL_DEPENDENCY_TYPES[Symbol.iterator](); !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = !0) {
        ...
        if (deps) for (var depName in deps) {
          var range = deps[depName], mostCommonRange = mostCommonRangeMap.get(depName);
          void 0 !== mostCommonRange && mostCommonRange !== range && semver.validRange(range) && errors.push({
  ...
  fix: function(error) {
    ...
    try {
      for (var _step2, _iterator2 = NORMAL_DEPENDENCY_TYPES[Symbol.iterator](); !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = !0) {
        ...
        deps && deps[error.dependencyName] && (deps[error.dependencyName] = error.mostCommonDependencyRange);
      }
    } catch (err) {
      _didIteratorError2 = !0, _iteratorError2 = err;
    } finally {
  ...
  validate: function(workspace, allWorkspaces) {
    ...
    if (peerDeps) for (var depName in peerDeps) if (devDeps[depName]) {
      if (semver__default.validRange(devDeps[depName]) && semver__default.validRange(peerDeps[depName]) && !sembear.upperBoundOfRangeAWithinBoundsOfB(devDeps[depName], peerDeps[depName])) {
        var _idealDevVersion = getMostCommonRangeMap(allWorkspaces).get(depName);
      ...
      var idealDevVersion = getMostCommonRangeMap(allWorkspaces).get(depName);
