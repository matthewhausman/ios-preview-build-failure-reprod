...
var getMostCommonRangeMap = weakMemoize(function getMostCommonRanges(allPackages) {
  var dependencyRangesMapping = new Map();
        ...
        if (deps) {
          for (var depName in deps) {
            ...
            if (!allPackages.has(depName)) {
              ...
              var dependencyRanges = dependencyRangesMapping.get(depName) || {};
              var specifierCount = dependencyRanges[depSpecifier] || 0;
  ...
  var mostCommonRangeMap = new Map();
  var _iteratorNormalCompletion3 = true;
  var _didIteratorError3 = false;
  var _iteratorError3 = undefined;

  try {
    for (var _iterator3 = dependencyRangesMapping[Symbol.iterator](), _step3; !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done); _iteratorNormalCompletion3 = true) {
      var _step3$value = _slicedToArray(_step3.value, 2),
          _depName = _step3$value[0],
          specifierMap = _step3$value[1];

      var specifierMapEntryArray = Object.entries(specifierMap);

      var _specifierMapEntryArr = _slicedToArray(specifierMapEntryArray, 1),
          first = _specifierMapEntryArr[0];

      var maxValue = specifierMapEntryArray.reduce(function (acc, value) {
        if (acc[1] === value[1]) {
          ...
          var highestRange = sembear.highest([acc[0], value[0]]);
...
var EXTERNAL_MISMATCH = makeCheck({
  validate: function validate(workspace, allWorkspace) {
    var errors = [];
    var mostCommonRangeMap = getMostCommonRangeMap(allWorkspace);
    var _iteratorNormalCompletion = true;
    var _didIteratorError = false;
    var _iteratorError = undefined;
      ...
      for (var _iterator = NORMAL_DEPENDENCY_TYPES[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
        ...
        if (deps) {
          for (var depName in deps) {
            ...
            var mostCommonRange = mostCommonRangeMap.get(depName);
  ...
  validate: function validate(workspace, allWorkspaces) {
    ...
    if (peerDeps) {
      for (var depName in peerDeps) {
        if (!devDeps[depName]) {
          var highestRanges = getMostCommonRangeMap(allWorkspaces);
        ...
        semver__default.validRange(peerDeps[depName]) && !sembear.upperBoundOfRangeAWithinBoundsOfB(devDeps[depName], peerDeps[depName])) {
          var _highestRanges = getMostCommonRangeMap(allWorkspaces);
