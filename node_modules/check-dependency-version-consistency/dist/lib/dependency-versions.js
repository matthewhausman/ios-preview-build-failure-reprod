 ...
 * }
 */
export function calculateVersionsForEachDependency(packages) {
    const dependenciesToVersionsSeen = new Map();
    for (const package_ of packages) {
        recordDependencyVersionsForPackageJson(dependenciesToVersionsSeen, package_);
    }
    return dependenciesToVersionsSeen;
}
function recordDependencyVersionsForPackageJson(dependenciesToVersionsSeen, package_) {
...
export function filterOutIgnoredDependencies(mismatchingVersions, ignoredDependencies, ignoredDependencyPatterns) {
    ...
    for (const ignoredDependencyPattern of ignoredDependencyPatterns) {
        if (!mismatchingVersions.some((mismatchingVersion) => ignoredDependencyPattern.test(mismatchingVersion.dependency))) {
            throw new Error(`Specified option '--ignore-dep-pattern ${ignoredDependencyPattern}', but no matching dependencies with version mismatches detected.`);
        }
    }
...
export function fixMismatchingVersions(packages, mismatchingVersions) {
    const fixed = [];
    const notFixed = [];
    ...
    for (const mismatchingVersion of mismatchingVersions) {
        ...
        for (const package_ of packages) {
            if (package_.packageJson.devDependencies &&
                ...
                package_.packageJson.devDependencies[mismatchingVersion.dependency] !==
                    fixedVersion) {
                writeDependencyVersion(package_.pathPackageJson, package_.packageJsonEndsInNewline, 'devDependencies', mismatchingVersion.dependency, fixedVersion);
                isFixed = true;
            }
            if (package_.packageJson.dependencies &&
                ...
                package_.packageJson.dependencies[mismatchingVersion.dependency] !==
                    fixedVersion) {
                writeDependencyVersion(package_.pathPackageJson, package_.packageJsonEndsInNewline, 'dependencies', mismatchingVersion.dependency, fixedVersion);
                isFixed = true;
            }
            if (package_.packageJson.resolutions &&
                ...
                package_.packageJson.resolutions[mismatchingVersion.dependency] !==
                    fixedVersion) {
                writeDependencyVersion(package_.pathPackageJson, package_.packageJsonEndsInNewline, 'resolutions', mismatchingVersion.dependency, fixedVersion);
                isFixed = true;
            }
        }
        if (isFixed) {
