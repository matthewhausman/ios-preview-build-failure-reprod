...
export function getPackages(root, ignorePackages, ignorePackagePatterns, ignorePaths, ignorePathPatterns) {
    ...
    for (const ignoredPackagePattern of ignorePackagePatterns) {
        ...
        !Package.some(packages, (package_) => ignoredPackagePattern.test(package_.name))) {
            throw new Error(`Specified option '--ignore-package-pattern ${ignoredPackagePattern}', but no matching packages detected in workspace.`);
        }
    }
    ...
    for (const ignoredPathPattern of ignorePathPatterns) {
        ...
        !Package.some(packages, (package_) => ignoredPathPattern.test(package_.pathRelative))) {
            throw new Error(`Specified option '--ignore-path-pattern ${ignoredPathPattern}', but no matching paths detected in workspace.`);
        }
    }
